...
def reproducibility(counts_a, counts_b):
    """Воспроизводимость на 2 сидах: max |ΔE(θ)| и в единицах σ."""
    ...
    return {
        ...
        "dE_sigma": dE / np.where(sig > 0, sig, 1.0),
        "max_dE_sigma": float(np.max(np.abs(dE) / np.where(sig > 0, sig, 1.0))),
    }
...
def chord_probs(theta, p):
    """P[pp,pm,mp,mm] для закона хорды P ∝ |s·a − t·b|^p (SPEC §2.6).
...
def saw_probs(theta):
    """P[pp,pm,mp,mm] для пилы E = 2θ/π − 1 (SPEC §1). θ в радианах."""
    theta = np.asarray(theta, dtype=np.float64)
    E = 2 * theta / np.pi - 1
...
def fit_chord_p(counts, thetas, p_lo=0.2, p_hi=6.0):
    """MLE p закона хорды: сетка + золотое сечение (чистый numpy).

    Возврат (p_hat, loglik_max).
    """
    counts = np.asarray(counts, dtype=np.float64)
    thetas = np.asarray(thetas, dtype=np.float64)

    grid = np.linspace(p_lo, p_hi, 60)
    lls = np.array([loglik_chord(counts, thetas, p) for p in grid])
    i = int(np.argmax(lls))
    lo = grid[max(i - 1, 0)]
    hi = grid[min(i + 1, len(grid) - 1)]

    # золотое сечение на [lo, hi]
    gr = (np.sqrt(5) - 1) / 2
    a, b = lo, hi
    c = b - gr * (b - a)
    d = a + gr * (b - a)
    fc = loglik_chord(counts, thetas, c)
    fd = loglik_chord(counts, thetas, d)
    for _ in range(60):
        if fc > fd:
            b, d, fd = d, c, fc
            c = b - gr * (b - a)
        ...
        if abs(b - a) < 1e-5:
            break
    p_hat = 0.5 * (a + b)
    return float(p_hat), loglik_chord(counts, thetas, p_hat)
...
def compare_models(counts, thetas):
    """Фит трёх моделей (пила, хорда-p, хорда-p=2) и сравнение по AIC (SPEC §4.3)."""
    counts = np.asarray(counts, dtype=np.float64)
    thetas = np.asarray(thetas, dtype=np.float64)
