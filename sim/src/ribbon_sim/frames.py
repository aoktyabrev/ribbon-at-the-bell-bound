...
def mirror_flip(q):
    """Зеркало q → q⊗j, j=180° вокруг x = (0,1,0,0): ось конца n→−n, исход (s,t)→(−s,−t).
    Для детерминированных зеркальных пар в инициализации (R3-geo, решение архитектора):
    делает ±-симметрию и маргиналы тождественными по построению."""
    return quat_mul(q, jnp.array([0.0, 1.0, 0.0, 0.0]))
