...
def e_cosserat(q, k_b, k_t, spinor=False):
    ...
    ДЕФЕКТ (decisions.md, R3): многошаговый градиент (conj→mul→log_map/atan2) рвёт
    ±-эквивариантность вдоль траектории релаксации ⇒ нарушает ±-симметрию исходов.
    Дефект ТОЧНОСТНО-НЕЗАВИСИМ (22.63σ одинаково в fp32 и fp64) — прежний диагноз
    «float32-неэквивариантность» ОПРОВЕРГНУТ мини-fp64-тестом; это дефект формулировки
    log_map. Оставлен для сверки старое/новое. atan2-Коссера deprecated; рабочая
    анизотропная энергия — cosserat_geo (R3-geo, 4223e20).
    """
