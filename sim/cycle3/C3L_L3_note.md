...
**2.2 Мост p ↦ S (CHSH).** В прямом законе (T3, C3B_hardening.json):
S(1)=1.66, **S(2)=2.828=2√2 (граница Цирельсона)**, S(3)=3.47, S(∞)=4
(PR-бокс, ступень). S(p) монотонно растёт по p; p=2 попадает В границу
Цирельсона. ⇒ хвост p>2 = СВЕРХ-цирельсоновский (сверхквантовый) вплоть
до PR при p→∞.

**2.3 Следствие (флагман).** В семействе с ВНУТРЕННИМ стирингом
no-signaling исключает весь неаффинный хвост (лемма §1) ⇒ исключает
всякое p≠2 ⇒ исключает весь сверх-цирельсоновский сектор p>2.
**Цирельсон-граница возникает как СЛЕДСТВИЕ запрета сигналинга внутри
конструкции — без information causality** [ARCH: контраст с Pawłowski
et al., Nature 461, 1101 (2009) — вывод Цирельсона из IC; сверю сам].
Оговорка честная: (i) мост §2.2 установлен в ПРЯМОМ законе (F1-тип);
для полной строгости S(p) считается в ТОМ ЖЕ внутренне-стиринговом
семействе, что и лемма — задача батареи/расширения; (ii) клейм в классе
конструкции (steering class), не общий; (iii) χ феноменологичен (долг
C3-B-mech). Формулировка до прохода батареи: «no-signaling forbids the
super-Tsirelson tail within the ribbon's steering class».

## 3. Батарея-дополнение L3 (ПОСЛЕ prereg-коммита; здесь — только план)
- Скан Δ(p, χ=0.5) внутренними ансамблями (сырое условное среднее, как
  C3-B), p∈{4, 6, 10, ∞(ступень f_∞=1{c>0})}, N=2·10⁶, критерии A4.
- Аналитика Йенсен-зазора рядом (обязательна; расхождение >2σ = стоп).
- Позитивность/сходимость как в hardening (алгебраический контроль знака).
- **Прогноз архитектора: Δ монотонно растёт по p, Δ(∞) максимален — 0.85.**
- Секция статьи 3 «Tsirelson from no-signaling» — ПОСЛЕ прохода.
